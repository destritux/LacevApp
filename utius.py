...
from nolds import dfa, lyap_e, lyap_r
    ...
    def startDir(self):
        ...
        for dicte in self.filepathC:
            if os.path.isdir(os.path.join(self.filePathR, str(dicte)) ): 
                dirPath1 =  os.path.join(self.filePathF, str(dicte)) 
                try:
                    os.mkdir(dirPath1)
                    msg.append(f"Creating Directory '{dicte}' in {self.filePathF} ... \n")
                except FileExistsError:
                    msg.append(f"Directory '{dicte}' already exists... \n")

                dirPath2 =  os.path.join(self.filePathFe, str(dicte)) 
                try:
                    os.mkdir(dirPath2)
                    msg.append(f"Creating Directory '{dicte}' in {self.filePathFe} ... \n")
                except FileExistsError:
                    msg.append(f"Directory '{dicte}' already exists... \n")
        return msg
...
class Features:
  ...
  def startFeature(self, dataP, minCut, timeStart, dateStart, classe, nameClass, SampleRate, results,  text_box, fileN, progBar, stage=False, Date=0):
    """
    Parameters
    ----------
    dataP : ndarray
    ...
    if(totalData >= minu): # To cut the series, the number of points must be greater than the amount of cut
      ...
      for i in range(1,numCut+1):  
          print(fileN, str(i)+' de '+str(numCut))         
          ...
          if (len(dataQ) > 100) and (float(vzero) > 0.0): # the cut series must have more than 100 points and not have the average value zeroed
            ...
            lyapC = lyap_e(numpyArray)
            chaos = lyap_r(numpyArray, min_tsep = 1305)
            ...
            if type(timeStart) == int: minuted=cminu
            else: 
                timeStartP = list(map(int, timeStart[starts:final]))
                timeStartP = sum(timeStartP)/len(timeStartP)
                minuted = int(timeStartP)
                minuted =  self._setTimeMinu( minuted)
            
            if type(dateStart) == int: dated='0/00/0000' 
            else: dated = dateStart[starts]

            if stage == False: obj = [ classe, nameClass, minuted, dated]
            else: obj = [ classe, nameClass, minuted, dated, stage]
            cai=0
            for ai in lyapC:
                if ai > 0:
                    cai += 1
            ...
            obj += [
                     float(abs(apenC)), float(abs(dfaC)), float(cai), float(chaos), float(abs(abpLow)),float(abs(abpDelta)),float(abs(abpTheta)), float(abs(abpAlpha)),  float(abs(abpBeta)),       
            ...
            cminu += minCut
            cminu =  self._setTimeMinu( cminu)
            
    return results
  def settimeHor(self, dataP, minuCut):
    Time= []
    horaA=0
    for i in dataP['HORA']:
        tsplit=str(i).split(':')
        hora = int(tsplit[0].replace(' ', '').replace('\\', '').replace('n', '').rstrip().lstrip())
        if hora < 24:
            minuto=int(str(tsplit[1]).replace('\\', '').replace('n', '').replace(' ', '').rstrip().lstrip())
            if minuto < 10:
                minuto = str('0'+str(minuto))
            horaA = str(str(hora)+str(minuto))
            ...
            if horaA == 0:
                ...
                horaA = int(horaA) + minuCut
                horaA =  self._setTimeMinu(horaA)
                Time.append(str(horaA))    
    
    return Time

  def _setTimeMinu(self, cminu):
    """
    Parameters
    ----------
    cminu: int
    ...
    if cminu > 60 and cminu < 100: cminu=100
    ...
    elif cminu > 2359: cminu=0
    return cminu
      
  
  def salveDf(self, results, classe, stage=False, mode=0):  
    if stage == False: colmns = ['classe', 'nameClass', 'minuted', 'date']
    else: colmns = ['classe', 'nameClass', 'minuted','date', 'stage']
    
    colmns += ['apen', 'dfa', 'chaos_cont_dim', 'lyap','abp_low','abp_delta','abp_theta', 'abp_alpha','abp_beta',
    ...
    if mode==0:     
        ...
        classe=str(classe).split('\\')
        pathDf = self.pathFeatures+'\\'+str(classe[0])+'\\df'+str(classe[1])+'.csv'
        try:
            data.to_csv(pathDf)
            return data
        except Exception as er:
...
class Filter: 
    ...
    def __init__(self, HPF=0.05, LPF=32,  order=4):
        """
        Parameters
        ----------
        HPF : float
...
class Plots:
    ...
    def __init__(self, SMALL_SIZE = 12, MEDIUM_SIZE = 14, BIGGER_SIZE = 20, FONT_SIZE = 12) -> None:
        global  GfilePathR, GfilePathF, GfilePathG, GfilePathFe
    ...
    def plotTDAF(self, filepathC,text_box, sample_rate=50):
        ...
        sns.lineplot(data=dados, x="minuted", y="electrome_mean",hue="nameClass", ax=ax1, palette='viridis')  
        sns.lineplot(data=dados, x="minuted", y="electrome_min",hue="nameClass", ax=ax2, palette='viridis')
        sns.lineplot(data=dados, x="minuted", y="electrome_max",hue="nameClass", ax=ax3, palette='viridis')
        sns.lineplot(data=dados, x="minuted", y="electrome_variance",hue="nameClass", ax=ax4, palette='viridis')     

        sns.lineplot(data=dados, x="minuted", y="fft_mean",hue="nameClass", ax=ax5, palette='autumn')  
        sns.lineplot(data=dados, x="minuted", y="fft_min",hue="nameClass", ax=ax6, palette='autumn')
        sns.lineplot(data=dados, x="minuted", y="fft_max",hue="nameClass", ax=ax7, palette='autumn')
        sns.lineplot(data=dados, x="minuted", y="fft_variance",hue="nameClass", ax=ax8, palette='autumn')   
        ...
        ax1.legend(loc='best', framealpha=0.1, title=None)
        ...
        sns.lineplot(data=dados, x="minuted", y="psd_mean",hue="nameClass", ax=ax1, palette='viridis')  
        sns.lineplot(data=dados, x="minuted", y="psd_min",hue="nameClass", ax=ax2, palette='viridis')
        sns.lineplot(data=dados, x="minuted", y="psd_max",hue="nameClass", ax=ax3, palette='viridis')
        sns.lineplot(data=dados, x="minuted", y="psd_variance",hue="nameClass", ax=ax4, palette='viridis')     

        sns.lineplot(data=dados, x="minuted", y="abp_low",hue="nameClass", ax=ax5, palette='autumn')  
        sns.lineplot(data=dados, x="minuted", y="abp_delta",hue="nameClass", ax=ax6, palette='autumn')
        sns.lineplot(data=dados, x="minuted", y="abp_theta",hue="nameClass", ax=ax7, palette='autumn') 
        sns.lineplot(data=dados, x="minuted", y="abp_alpha",hue="nameClass", ax=ax8, palette='autumn') 
        sns.lineplot(data=dados, x="minuted", y="abp_beta",hue="nameClass", ax=ax9, palette='autumn') 
        ...
        title1='TDAF-ApEn'
        title2='TDAF-DFA'
        title3='TDAF-Lyapunov exponent'
        text_box.insert(1.0, "Creating Ploat "+title1+" "+title2+". Please wait.\n")  
        text_box.pack()
        sns.lineplot(data=dados, x="minuted", y="apen",hue="nameClass", ax=ax1, palette='viridis')  
        sns.lineplot(data=dados, x="minuted", y="dfa",hue="nameClass", ax=ax2, palette='autumn') 
        sns.lineplot(data=dados, x="minuted", y="lyap",hue="nameClass", ax=ax3, palette='autumn') 

        plt.setp(ax1, title=title1)    
        plt.setp(ax2, title=title2)    
        plt.setp(ax3, title=title3)     
        ax1.spines['right'].set_visible(False)
        ax1.spines['top'].set_visible(False)
        ax2.spines['right'].set_visible(False)
        ax2.spines['top'].set_visible(False)
        ...
        ax1.legend(loc='best', framealpha=0.1, title=None)
        ax2.legend(loc='best', framealpha=0.1, title=None)
        ax3.legend(loc='best', framealpha=0.1, title=None)
